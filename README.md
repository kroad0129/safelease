...
- 자동화된 단위·통합·프런트엔드 테스트 파일은 현재 저장소에 없습니다.
- `run_backend_check.py`로 전체 흐름을 수동 점검할 수 있습니다.
- 프런트엔드는 `npm run lint`와 `npm run build` 명령을 제공합니다.
- 법률 전문가가 검증한 정답 데이터셋이 없어 추출 정확도, 검색 정확도, 법률 판단 정확도를 주장하지 않습니다.
- 기존 실행 기록의 후보 제외 비율은 정확도 향상률이 아닙니다.

## 알려진 한계

- 실제 배포·운영 환경과 CI/CD 구성이 없습니다.
- 분석이 동기식으로 실행되어 외부 API 지연 시 요청 시간이 길어질 수 있습니다.
- 업로드 파일은 확장자와 빈 파일 여부만 검사하며 MIME, 용량, 악성 파일 검사는 하지 않습니다.
- 인증과 사용자별 기록 접근 제어가 없습니다.
- `/outputs`가 정적 경로로 노출되며 산출물의 만료·삭제 정책이 없습니다.
- 외부 서비스 장애 대응은 일부 상태 분류와 Selenium 재시도에 한정됩니다.
- DB 스키마 생성 및 seed 데이터 준비 절차가 저장소에 포함되어 있지 않습니다.
- 자동 테스트와 고정된 Python 의존성 버전이 없습니다.
