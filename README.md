...
현재 저장소에는 API 키 없이 전체 화면을 재현하는 고정 데모 모드가 없습니다. 향후 가상 계약서를 실제 파이프라인으로 분석한 비식별 결과를 fixture로 보관하고, 외부 API와 LLM을 호출하지 않아도 같은 결과 화면을 확인할 수 있도록 구성할 예정입니다.

데모 데이터는 임의로 만든 성과 수치가 아니라 다음과 같은 실제 파이프라인 산출물의 비식별 사본으로 구성합니다.
...
<!-- TODO: fixture 기반 Demo Mode 구현 후 실행 방법과 데모 화면을 추가합니다. -->
