...
[프로젝트 README](../README.md) · [API 명세](../docs/API.md) · [아키텍처](../docs/ARCHITECTURE.md)

SafeLease 백엔드는 주택 임대차 계약서 PDF를 업로드받아 계약 내용을 구조화하고, 주소·중개업·임대료를 검증한 뒤 법령 RAG와 특약 분석을 결합해 계약 검토 결과를 생성한다. 프런트엔드는 이 결과와 PDF 하이라이트, 분석 기록, 질의응답을 표시하며 분석 로직과 산출물 생성은 백엔드가 담당한다.
